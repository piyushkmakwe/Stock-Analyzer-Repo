...
FAIL=0
for t in smoke-test engine-test pdf-test qual-test accuracy-test why-test library-test pin-test targets-test batch3-test ledger-test horizon-test exclusions-test; do
  echo "── $t ──────────────────────────────"
  node "$t.js"
  [ $? -ne 0 ] && FAIL=1
  echo
