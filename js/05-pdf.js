    ...
    <div style="flex:1;border:2px solid ${ratingTone};border-radius:10px;padding:13px 16px">
      ...
      <div style="border-top:1px solid ${PDF_GRID};margin-top:10px;padding-top:9px">
        ...
        <div style="font-size:8.5px;color:${PDF_INK2};line-height:1.65">
          ...
          ${why.target?`<strong>2.</strong> 5-yr score: base target = EPS ${pdfINR(why.target.eps,2)}${why.target.epsBasis==='cycle-normalized'?' (cycle-normalized: 5-yr average margin × today’s revenue — TTM earnings sit far from mid-cycle in this sector)':''} × (1+${(why.target.g*100).toFixed(1)}%)⁵ × exit P/E ${why.target.exitPE.toFixed(1)} = ${pdfINR(why.target.base5)}; expected value = 25% bear ${pdfINR(why.target.bear5)} + 50% base + 25% bull ${pdfINR(why.target.bull5)}${why.target.div5?` + dividends ${pdfINR(why.target.div5)}`:''}; ÷ price ${pdfINR(why.target.cmp)} = <strong>score ${why.target.score.toFixed(2)}</strong>${why.target.sev>1?` — bear case hardened ×${why.target.sev.toFixed(2)} because the revenue history is lumpy`:''}.<br>`:''}
